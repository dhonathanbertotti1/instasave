...
export default function BatchDownload() {
      ...
      validLinks.map(async (entry) => {
        try {
          ...
          setLinks((prev) =>
            prev.map((l) =>
              l.id === entry.id
                ? { ...l, status: "error", error: err instanceof Error ? err.message : "Erro" }
                : l
            )
          );
        }
  ...
  return (
    <section id="download-multiplo" className="py-20 px-4 border-t border-white/5">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="text-center mb-10">
          ...
          <h2 className="text-3xl md:text-4xl font-bold mb-3">
            Baixe <span className="gradient-text">Vários Links</span> de Uma Vez
          </h2>
          <p className="text-gray-400">
            Cole vários links do Instagram, busque todas as mídias e baixe tudo em um único arquivo <strong className="text-white">.zip</strong>.
          </p>
        </div>

        {/* Área de colagem em massa */}
        <div className="glass-card rounded-2xl p-5 mb-4">
          <label className="text-xs text-gray-500 uppercase tracking-wider mb-2 block">Cole vários links (um por linha)</label>
          <textarea
            ref={textareaRef}
          ...
          />
          <p className="text-xs text-gray-600 mt-1">Ou adicione os links individualmente abaixo ↓</p>
        </div>
            ...
            <div key={entry.id} className="flex items-center gap-2">
              <div className="relative flex-1">
                <input
                  ...
                  placeholder={`Link do Instagram #${index + 1}`}
              ...
              {entry.status === "success" && entry.data && (
                <span className="text-xs text-green-400 whitespace-nowrap">
                  {entry.data.items.length} {entry.data.items.length === 1 ? "arquivo" : "arquivos"}
                </span>
              )}
              {entry.status === "error" && (
          ...
          >
            {loadingCount > 0 ? (
              <>
                ...
                Buscando {loadingCount} link{loadingCount !== 1 ? "s" : ""}...
              </>
            ) : (
              <>
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <>
                ...
                {totalItems > 0 && (
                  <span className="ml-1 px-2 py-0.5 rounded-full bg-white/20 text-xs">
                    {totalItems} {totalItems === 1 ? "arquivo" : "arquivos"}
                  </span>
                )}
              </>
            )}
        ...
        {zipping && (
          <div className="mt-4 fade-in">
            ...
            <p className="text-xs text-gray-500 mt-2 text-center">
              Baixando {totalItems} arquivos e empacotando no ZIP...
            </p>
          </div>
        )}
        ...
        {zipDone && !zipping && (
          <div className="mt-4 p-3 rounded-xl bg-green-500/10 border border-green-500/20 text-green-400 text-sm text-center fade-in">
            ✓ Arquivo ZIP baixado com {totalItems} mídias
          </div>
        )}
      </div>
    </section>
